...
def clean_df():
    ...
    df = pd.read_csv(ROOT / 'Spots in tracks statistics.csv', na_values="None", delimiter='\t', header=0,
                     usecols=['ID', 'TRACK_ID', 'POSITION_X', 'POSITION_Y', 'FRAME']).dropna().astype(int)
    df.to_csv(ROOT / 'statistics_clean.csv', index=False)
...
if __name__ == '__main__':
    df = pd.read_csv(ROOT / 'statistics_clean.csv')
    df = add_mean_std(df, n_frames=None, verbose=True)
    # df.to_csv(ROOT / 'statistics_mean_std.csv', index=False, float_format='%.3f')

    # df = pd.read_csv(ROOT / 'statistics_mean_std.csv')
    ...
    df.to_csv(ROOT / 'statistics_mean_std_.csv', index=False, float_format='%.3f')
